...
if [ ! -f "pysidedeploy.spec" ]; then
  ...
  sed -i 's|^extra_args = .*|extra_args = --quiet --noinclude-qt-translations --include-module=mido.backends.rtmidi --include-data-dir=ui/icons=ui/icons --include-data-files=ui/style.qss.template=ui/style.qss.template|' pysidedeploy.spec
